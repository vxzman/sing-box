...
On FreeBSD, this value selects the dedicated FIB (routing table) used for
loop prevention. See the deployment notes below.

`2022` is used by default.

!!! note "FreeBSD"

    When using `auto_route` on FreeBSD, sing-box uses a dedicated FIB
    (selected by `iproute2_table_index`) for loop prevention. This
    requires the following tunables in `/boot/loader.conf` and a reboot:
    ...
    The outbound sockets of sing-box are bound to the dedicated FIB
    automatically.
